...
public class RS2ProtocolDecoder extends ReplayingDecoder<ByteBuf> {
	
	/**
	 * The incoming packet opcode.
	 */
	...
	/**
	 * The expected packet sizes for the incoming packets.
	 */
	 ...
	 */
	public RS2ProtocolDecoder(Player player) {
		this.player = player;
	}
	...
	protected void decode(ChannelHandlerContext context, ByteBuf buffer, List<Object> out) throws Exception {
		if (opcode == -1) {
			...
			opcode = (opcode - player.getDecryption().getNextValue()) & 0xFF;
		...
		if (buffer.readableBytes() >= size) {
			...
			try {
				PacketHandler.handleReceivedPacket(player, new Packet(opcode, payload));
			} finally {
				opcode = -1;
				size = -1;
			}
